...
class DiscoTestCase(helper.CPWebCase):
    ...
    def setup_server():
        cherrypy.tree.graft(
            SessionMiddleware(DiscoTestCase.application,
                              proxy_server_conf_default.SESSION_OPTS),
            '/')
    ...
    def test_discovery(self):
        ...
        if "returnIDParam" in req:
            return_id_param = req["returnIDParam"][0]
        url = req["return"][0] + "&" + urllib.parse.urlencode(
            {return_id_param: "http://test.idp.se:1111/TestIdP.xml"})
        self.getPage(url)
        self.assertStatus('303 See Other')
        location = None
